...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (is_null($request->items) || !is_array($request->items)) {
            \Log::error('Order items are not an array or are null', [
                'items' => $request->items,
                'order_id' => $order_id,
            ]);
            return response()->json(['error' => 'Order items must be a valid array.'], 422);
        }
